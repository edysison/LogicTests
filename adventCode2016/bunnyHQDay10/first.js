...
const test = (data) => {
  ...
  while (actions.length > 0) {
    actionsLoop: for (let j = 0; j < actions.length; j += 1) {
      ...
      if (
        bots[giverBotNumber] &&
        bots[giverBotNumber].includes("61") &&
        bots[giverBotNumber].includes("17")
      ) {
        return actions[j];
      }
